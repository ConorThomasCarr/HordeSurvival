...
public class RoamState : BaseState
{
    ...
    {
        if (!OnChangeDestination() && CharacterConfig.NavmeshConfig.Agent.remainingDistance <= 2)
        {
            return true;
        }
        if (OnChangeDestination())
        {
            destination = CharacterConfig.NavmeshConfig.Agent.destination;
        }
        
        return false;
    }
    
    private bool OnChangeDestination()
    {
        return CharacterConfig.NavmeshConfig.Agent.destination != destination;
