...
namespace AIUndeadEnemy
{
    public class ZombieNpc : MonoBehaviour, IUndeadNpc
    {
        ...
        private void Update()
        {
            if (ActionPhase == FsmActionPhase.Combat)
            {
                _agent.updateRotation = false;

                transform.GetChild(0).LookAt(_zombieInterface.Player.transform.position);

                if (_weaponInventory.HeldWeapon.WeaponInterface.magazineSize != 0)
                {
                    if (_weaponInventory.HeldWeapon.WeaponInterface.WeaponSpread > _weaponInventory.HeldWeapon.WeaponInterface.WeaponConfig.CoreConfig.WeaponSpreadMinAngle && shoot != true)
                    {
                        _weaponInventory.HeldWeapon.AimIsInProgress.Invoke();
                    }

                    if (_weaponInventory.HeldWeapon.WeaponInterface.WeaponSpread <= _weaponInventory.HeldWeapon.WeaponConfig.CoreConfig.WeaponSpreadMaxAngle && shoot)
                    {
                    ...
                    if (_weaponInventory.HeldWeapon.WeaponInterface.WeaponSpread >= _weaponInventory.HeldWeapon.WeaponInterface.WeaponConfig.CoreConfig.WeaponSpreadMaxAngle && shoot) 
                    {
                        shoot = false;
                    }
                ...
                if ( _weaponInventory.HeldWeapon.WeaponInterface.magazineSize == 0 && !_reload)
                {
